...
class TCPProxy extends EventEmitter {
  ...
  createServer() {
    ...
    this._server.listen(this.port, () => {
      logger.info('[proxyworker] tcp server listen at %s', this.port);
    });
  ...
  closeServer(cb) {
    if (this._server) {
      ...
      this._server.close(() => {
        logger.info(`[proxyworker] tcp server port ${this.port} had closed.`);
        this._server = null;
        cb();
      });
    }
  ...
  createServerSockets(serverSocket) {
    ...
    this.serverSocket.on('error', error => {
      ...
      logger.error('[proxyworker] server socket error', error);
    ...
    this.serverSocket.on('close', error => {
      logger.info('[proxyworker] server socket closed.');
  ...
  createClientSockets(target) {
    logger.info('[proxyworker] create client socket ing...');
    this.clientSocketConnected = false;
    ...
    const clientSocket = this.clientSocket = new net.createConnection({
      ...
      logger.info('[proxyworker] client socket started.');
    ...
    clientSocket.on('error', () => {
      this.clientSocketConnected = false;
      logger.error(`[proxyworker] could not connect to client at host ${this.target.host}:${this.target.port}`);
    });
    ...
    clientSocket.on('close', error => {
      logger.info('[proxyworker] client socket closed.');
  ...
  _socketHandler(serverSocket) {
    logger.info(`[proxyworker] receive new socket connection from ${serverSocket.remoteAddress}:${serverSocket.remotePort}`);
