...
class TCPProxy extends EventEmitter {
  ...
  createServer() {
    ...
    this._server.listen(this.port, () => {
      console.log('[proxyworker] tcp server listen at %s', this.port);
    });
  }
  ...
  closeServer(cb) {
    if (this._server) {
      ...
      this._server.close(() => {
        console.log(`[proxyworker] tcp server port ${this.port} had closed.`);
        this._server = null;
        cb();
      });
    }
  ...
  createServerSockets(serverSocket) {
    ...
    this.serverSocket.on('error', error => {
      ...
      console.error('[proxyworker] server socket error', error);
    ...
    this.serverSocket.on('close', error => {
      console.log('[proxyworker] server socket closed.');
  ...
  createClientSockets(target) {
    console.log('[proxyworker] create client socket ing...');
    this.clientSocketConnected = false;
    ...
    const clientSocket = this.clientSocket = new net.createConnection({
      ...
      console.log('[proxyworker] client socket started.');
    ...
    clientSocket.on('error', () => {
      this.clientSocketConnected = false;
      console.error(`[proxyworker] could not connect to client at host ${this.target.host}:${this.target.port}`);
    });
    ...
    clientSocket.on('close', error => {
      console.log('[proxyworker] client socket closed.');
  ...
  _socketHandler(serverSocket) {
    console.log(`[proxyworker] receive new socket connection from ${serverSocket.remoteAddress}:${serverSocket.remotePort}`);
